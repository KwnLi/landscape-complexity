...
base.v.delta <- ggplot(dat_wider, aes(`Baseline ES [/ha]`, `Δ ES [/ha]`)) + geom_point() + 
  xlab("Baseline ES per ha") + ylab("Increase in ES per ha") +
  geom_smooth(method = "gam") + facet_wrap(~ES, scales = "free") + 
  ggtitle("ES baseline vs \nES change in ES") + egg::theme_article()
...
base.v.perc <- ggplot(dat_wider, aes(`Baseline ES [/ha]`, `% Δ ES`)) + 
  ...
  ylab("% improvement from baseline") +
  egg::theme_article()
...
# cowplot::plot_grid(base.v.margin, base.v.margin.base, base.v.permargin)

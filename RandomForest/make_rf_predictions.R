...
pred.pollinators_pc <- Predictor$new(
  model = pollinators_pc.rf,
  data = dat,
  y = "pc_pol_incr.margin",
  predict.function = pfun
)

pred.nexp_pc <- Predictor$new(
  model = n_export_pc.rf,
  data = dat,
  y = "pc_nexp_red.margin",
  predict.function = pfun
)

pred.sed_pc <- Predictor$new(
  model = sed_pc.rf,
  data = dat,
  y = "pc_sed_red.margin",
  predict.function = pfun
...
pred.qb_pc <- Predictor$new(
  model = qb_pc.rf,
  data = dat,
  y = "pc_qb_incr.margin",
  predict.function = pfun
)
save(list = ls(pattern = "^pred.+pc$"), file = "data/ES-RF3pc-preds.RDA")
